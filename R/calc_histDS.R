...
#' @export
calc_histDS <- function(data_name, amt_trees) {
  ...
  if (dropout_rate == 0) {
    ...
    last_tr_tree <- eval(parse(text = paste0(data_name, "_tree_", amt_trees)),
                         envir = parent.frame())
    ...
    amt_removed_trees <- rbinom(1, amt_trees, dropout_rate)
    
    if (amt_removed_trees == 0) {
      amt_removed_trees <- 1
    }
    
    removed_trees <- sample(amt_removed_trees, amt_trees)
    
    if (dropout_rate <= 0.5) {
      if (!amt_trees %in% removed_trees) {
        last_tr_tree <- eval(parse(text = paste0(data_name, "_tree_", amt_trees)),
                             envir = parent.frame())
        data_by_row <- split(training_data, seq_len(nrow(training_data)))
        new_pred <- sapply(X = data_by_row, FUN = tree_evaluationDS, last_tr_tree,
